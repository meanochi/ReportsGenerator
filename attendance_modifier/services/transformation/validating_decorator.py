...
class ValidatingStrategyDecorator(BaseTransformationStrategy):
    ...
    def _validate(self, row: AttendanceRow) -> None:
        ...
        if work_hours > _MAX_WORK_HOURS:
            raise TransformationError(
                f"[{row.date}] work duration {work_hours:.1f}h exceeds maximum {_MAX_WORK_HOURS}h."
            )

        if row.break_minutes is not None and row.break_minutes > _MAX_BREAK_MINUTES:
            raise TransformationError(
                f"[{row.date}] break_minutes ({row.break_minutes}) exceeds logical bound of {_MAX_BREAK_MINUTES}."
            )
